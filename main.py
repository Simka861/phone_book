...
class Main(tk.Frame):
    ...
    def records(self, name, tel, email):
        self.db.insert_data(name, tel, email)   #Вызвали функцию добавления данных в базу данных
        self.view_records()                     #Вызвали функцию обновления данных и вывода данных на окно

    def view_records(self):
        self.db.cursor.execute("SELECT * FROM db")                                      #Запрос: Выбираем все данные из таблицы
    ...
    def delete_records(self):
        for selection_items in self.tree.selection():
            self.db.cursor.execute(                                                     # Запрос на удаление строки с таким-то id
                "DELETE FROM db WHERE id=?", (self.tree.set(selection_items, "#1"))     # Передаём аргумет на место "?"
            )
        self.db.conn.commit()                                                           # Сохраняем запрос
        self.view_records()                                                             # Вызываем функцию класса
...
class Child(tk.Toplevel):                               # Дочерний класс
    ...
    def init_child(self):
        self.title("Добавить")                          #Указали название заголовка
...
class Update(Child):
    ...
    def init_edit(self):
        self.title("Редактирование контакта")               #Указали название заголовка
...
class Search(tk.Toplevel):
    ...
    def init_search(self):
        self.title("Поиск контакта")                        #Указали название заголовка
...
class DB:
    ...
    def insert_data(self, name, tel, email):                                            # Метод для добавленияя наших данных в таблицу
        self.cursor.execute(                                                            # Запрос на создание бд
            """INSERT INTO db(name, tel, email) VALUES(?, ?, ?)""", (name, tel, email)  # передаём аргументы на места '?'
        )
        self.conn.commit()               # Сохраняем запрос
...
if __name__ == "__main__":
    ...
    root.title("Телефонная книга")       # Установили заголовок экземпляра Tk
    root.geometry("665x450")             # Установили размер экземпляра Tk
